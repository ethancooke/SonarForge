...
struct TroubleshootingView: View {
    ...
    private let items: [Item] = [
        ...
        Item(symptom: "Some apps aren't affected",
             advice: "Apps using exclusive audio access or certain DRM paths can bypass the system tap. Netflix in a browser is confirmed to work; some protected players may not. This is a macOS limitation of the driverless approach."),
        Item(symptom: "Audio glitch when switching devices",
             advice: "A brief gap during a device switch is expected — the capture path is rebuilt for the new device (with a fade, not a click). If audio doesn't come back within a couple of seconds, use Reset Audio Engine."),
        Item(symptom: "Sound is fine but CPU seems high",
             advice: "Spectrum analysis only runs while the graph is on screen. Close the main window (the menu bar item keeps working) or untick Pre and Post to disable analysis entirely."),
    ]
